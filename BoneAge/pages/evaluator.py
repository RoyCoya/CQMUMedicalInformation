...
def evaluator(request, task_id):
    ...
    next_task = None
    if task.closed:
        try: pre_task = Task.objects.filter(standard=preference.standard).filter(allocated_to=request.user, closed=True).filter(closed_date__gt=task.closed_date).order_by('closed_date').first()
        except: pass
        try: next_task = Task.objects.filter(standard=preference.standard).filter(allocated_to=request.user, closed=True).filter(closed_date__lt=task.closed_date).order_by('closed_date').last()
        except: pass
    else:
        try: pre_task = Task.objects.filter(standard=preference.standard).filter(allocated_to=request.user, closed=False).filter(id__lt=task.id).last()
        except: pass
        try: next_task = Task.objects.filter(standard=preference.standard).filter(allocated_to=request.user, closed=False).filter(id__gt=task.id).first()
        except: pass
