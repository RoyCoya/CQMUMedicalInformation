from django.urls import path
from . import views

# app_name = 'BoneAge'

pages = [
	# 医生主页（未完成任务）
	path('page/<int:page_number>/', views.page_index, name='BoneAge_index'),
	# 医生主页（已完成任务）
	path('finished/page/<int:page_number>/', views.page_finished_tasks, name='BoneAge_finished_tasks'),
	# 管理员主页
	path('admin/', views.page_admin, name='BoneAge_admin'),
	# 评分器
	path('evaluator/<int:task_id>/', views.page_evaluator, name='BoneAge_evaluator'),
	# 骨龄记录库
	path('library/', views.page_library, name='BoneAge_library'),
    path('library/navigator/', views.page_navigator, name='library_navigator')
]

apis = [
	# 个人偏好
	path('preference/standard/', views.api_preference_save, name='api_BoneAge_preference_save'),
	# 评分器
	path('evaluator/offset/', views.api_save_image_offset, name='api_BoneAge_save_image_offset'),
	path('evaluator/bone_detail/', views.api_modify_bone_detail, name='api_BoneAge_modify_bone_detail'),
	path('evaluator/bone_postion/', views.api_modify_bone_position, name='api_BoneAge_modify_bone_position'),
	path('evaluator/task/report/', views.api_task_report, name='api_BoneAge_task_report'),
	path('evaluator/task/submit/', views.api_task_submit, name='api_BoneAge_task_submit'),
	path('evaluator/task/withdraw/', views.api_task_withdraw, name='api_BoneAge_task_withdraw'),
	path('evaluator/task/verify/', views.api_task_verify, name='api_BoneAge_task_verify'),
	path('evaluator/task/reject/', views.api_task_reject, name='api_BoneAge_task_reject'),
	path('evaluator/task/finish/', views.api_task_finish, name='api_BoneAge_task_finish'),
    path('evaluator/task/delete/', views.api_task_delete, name='api_BoneAge_task_delete'),
    path('evaluator/task/reopen/', views.api_task_reopen, name='api_BoneAge_task_reopen'),
	# path('evaluator/task/mark/', views.api_mark_task, name='api_BoneAge_mark_task'),
	# 骨龄记录库
	path('admin/dicom/upload/', views.api_upload_dcm, name='api_BoneAge_upload_dcm'),
	# 管理员后台
	path('admin/tasks/allocate/', views.api_allocate_tasks, name='api_BoneAge_allocate_tasks'),
    path('admin/tasks/delete/',views.api_delete_tasks,name='api_BoneAge_delete_tasks'),
	path('admin/export_bone_data/', views.api_export_bone_data, name='api_BoneAge_export_bone_data'),
]

urlpatterns = pages + apis